...
log() { echo "[tool_download_extras] $*" >&2; }

log "syncing vendored tools..."
if ! "${ROOT}/tool_sync.sh" "${ROOT}/tools.json"; then
...
find "${TOOLS_DIR}" -maxdepth 1 -type f \( \
  ...
  -name 'smtp-user-enum' -o \
  -name 'houndcollector.sh' \
\) -exec chmod +x {} + 2>/dev/null || true

chmod +x "${ROOT}/tool_sync.sh" "${ROOT}/serve.sh" "${ROOT}/tool_download_extras.sh" 2>/dev/null || true

log "tool_download_extras complete"
log "  tools:     ${TOOLS_DIR}/"
log "  accesschk: ${TOOLS_DIR}/accesschk.exe , ${TOOLS_DIR}/accesschk64.exe"
log "  serve:     ${ROOT}/serve.sh"
