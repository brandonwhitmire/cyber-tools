...
start_http() {
  if command -v python3 >/dev/null 2>&1; then
    log "HTTP  http://0.0.0.0:${HTTP_PORT}/  (root: ${SERVE_DIR})"
    (cd "$SERVE_DIR" && python3 -m http.server "$HTTP_PORT" --bind 0.0.0.0) &
    HTTP_PID=$!
    return 0
  fi
...
start_smb() {
  ...
  if command -v impacket-smbserver >/dev/null 2>&1; then
    smb_cmd=(impacket-smbserver "$SMB_SHARE" "$SERVE_DIR" -smb2support -username "$SMB_USER" -password "$SMB_PASS")
  elif command -v smbserver.py >/dev/null 2>&1; then
    smb_cmd=(smbserver.py "$SMB_SHARE" "$SERVE_DIR" -smb2support -username "$SMB_USER" -password "$SMB_PASS")
  elif python3 -c "import impacket.examples.smbserver" 2>/dev/null; then
    smb_cmd=(python3 -m impacket.examples.smbserver "$SMB_SHARE" "$SERVE_DIR" -smb2support -username "$SMB_USER" -password "$SMB_PASS")
  else
    log "WARN: impacket smbserver not found — HTTP only (install: pip install impacket)"
    return 0
  fi

  log "SMB   \\\\$(hostname -I 2>/dev/null | awk '{print $1}' || hostname)\\${SMB_SHARE}  (user: ${SMB_USER}, pass: ${SMB_PASS})"
  "${smb_cmd[@]}" &
  SMB_PID=$!
}
